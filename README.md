...
RabbitMQ-driven request processing, SignalR real-time notifications and a
configurable borrowing-expiration background job.
...
Library-System-API/
└── Library-System-API/
    ├── Controllers/           # One controller per documented category:
    │                          #   Books, BorrowingRequests, AdminActions, Notifications (+Auth)
    ├── Extensions/            # DI composition root, result→HTTP mapping, claims helpers, seeder
    ├── Middleware/            # Centralized exception handling (sanitized error envelope)
    ├── Business/              # Business layer
    │   ├── BackgroundJobs/    # BorrowingExpirationJob + configurable settings
    │   ├── DTOs/              # Auth / Books / Requests / Notifications contracts
    │   ├── Hubs/              # SignalR NotificationsHub (/hubs/notifications)
    │   ├── Interfaces/        # Service abstractions
    │   ├── Mappings/          # Manual entity→DTO mappings (no mapping library)
    │   ├── Messaging/         # RabbitMQ publisher, consumer (hosted service), settings
    │   ├── Notifications/     # Real-time dispatcher abstraction + SignalR implementation
...
| `Jwt` | `Secret` (min 32 chars), `Issuer`, `Audience`, `ExpiryMinutes` |
...
3. Start RabbitMQ (e.g., `docker run -p 5672:5672 rabbitmq:3`). If the broker is
   ...
   failures are logged.
4. `dotnet run --project Library-System-API` and open Swagger at `/swagger`.
...
approval/denial state transitions, notification inbox security and reminder
de-duplication.
